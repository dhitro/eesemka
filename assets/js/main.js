(function ($) {
    ...
    $(document).on("change", "#per_hal", function() {
        this.form.submit();
    });
    ...
    $( '.tambah-pengalaman' ).on( 'click', function(e) {
        e.preventDefault();
        var html = '';
        html += '<div class="row" id="inputFormRow">';
        html += '<div class="col-3">';
        html += '<input type="text" name="tahun[]" placeholder="Tahun" autocomplete="off">';
        html += '</div>';
        html += '<div class="col-6">';
        html += '<input type="text" name="pengalaman[]" placeholder="Masukkan Pengalaman" autocomplete="off">';
        html += '</div>';
        html += '<div class="col-3">';
        html += '<button id="removeRow" type="button" class="btn-sm btn-danger"> <i class="la la-trash-alt"></i> Hapus</button>';
        html += '</div>';
        html += '</div>';

        $('#pengalaman').append(html);
     ...
     $( '.tambah-keahlian' ).on( 'click', function(e) {
        e.preventDefault();
        var id = uniqId();
        var html = '';
        html += '<div class="col-3" id="inputFormRow">';
        html += '<div class="field-group field-file">';
        html += '<label for="s_'+id+'" class="preview">';
        html += '<input type="file" id="s_'+id+'" name="sertifikat_ahli[]" class="upload-file" data-max-size="1024">';
        html += '<img class="img_preview" alt="">';
        html += '<i class="la la-cloud-upload-alt"></i>';
        html += '</label>';
        html += '<div class="field-note">Maximum file size: 1 MB.<br> <button id="removeFile" type="button" class="btn-sm btn-danger"> <i class="la la-trash-alt"></i> Hapus</button></div>';
        html += '</div>';
        html += '</div>';
       

        $('#keahlian').append(html);
    ...
    $(document).on('click', '.removeFiles', function (e) {
        e.preventDefault();
        var th = $(this);
        var id = th.data('id');
        var href = $(this).attr('href');
        $.post(href,{id:id},function(response){ 
            th.closest('#inputFormRow').remove();
        });
    ...
    $(document).on('click', '.gantiFile', function (e) {
        ...
        $.post(href,{id:id},function(response){ 
            ...
            html += '<div class="col-3" id="inputFormRow">';
            html += '<div class="field-group field-file">';
            html += '<label for="foto" class="preview">';
            html += '<input type="file" id="foto" name="foto[]" class="upload-file" data-max-size="1024">';
            html += '<img class="img_preview" alt="">';
            html += '<i class="la la-cloud-upload-alt"></i>';
            html += '</label>';
            html += '<div class="field-note">Maximum file size: 1 MB.</div>';
            html += '</div>';
            html += '</div>';
            $('#fotoprofil').append(html);
        });
      
    });

     $( '.tambah-pendukung' ).on( 'click', function(e) {
        e.preventDefault();
        var id = uniqId();
        var html = '';
        html += '<div class="col-3" id="inputFormRow">';
        html += '<div class="field-group field-file">';
        html += '<label for="s_'+id+'" class="preview">';
        html += '<input type="file" id="s_'+id+'" name="sertifikat_pendukung[]" class="upload-file" data-max-size="1024">';
        html += '<img class="img_preview" alt="">';
        html += '<i class="la la-cloud-upload-alt"></i>';
        html += '</label>';
        html += '<div class="field-note">Maximum file size: 1 MB.<br> <button id="removeFile" type="button" class="btn-sm btn-danger"> <i class="la la-trash-alt"></i> Hapus</button></div>';
